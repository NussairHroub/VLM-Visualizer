...
#  Add argparse to accept command-line arguments with default values
parser = argparse.ArgumentParser(description="Run LLaVA-based visualization with a given image and prompt.")
parser.add_argument(
    "image_path_or_url", nargs="?", type=str, 
    default="https://github.com/open-compass/MMBench/blob/main/samples/MMBench/1.jpg?raw=true",
    help="Path or URL to the input image (default: sample image URL)"
)
parser.add_argument(
    "prompt_text", nargs="?", type=str, 
    default="What python code can be used to generate the output in the image?",
    help="Prompt text for the model (default: sample prompt)"
)
